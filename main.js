...
var NativeSlidesPlugin = class extends import_obsidian.Plugin {
  ...
  async showFileProperties() {
    const match = Object.entries(this.app.commands.commands).find(([, cmd]) => {
      const name = cmd.name ?? "";
      return name.toLowerCase().includes("properties") && /(?:show|display)/i.test(name) || name.includes("\u5C5E\u6027") && /(?:显示|打开)/.test(name);
    });
    if (match) {
