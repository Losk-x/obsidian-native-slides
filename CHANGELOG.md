...
- Slides mode now **auto-fits to a single screen**: the editor content scales proportionally (CSS `zoom`) to fill the viewport — no scrolling, no clipping — re-fitting automatically on window resize, Obsidian zoom and content changes.
