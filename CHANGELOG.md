...
- **Style templates**: five built-in looks for the Slides card **and** slides bar — _Dashed outline_ (default, the previous look), _Paper card_ (solid slide with soft shadow), _Minimal_ (no card boundary, flat canvas), _Accent edge_ (accent line on card top and bar) and _Frosted glass_ (translucent, blurred surfaces over a tinted backdrop). Pick one under Settings → Style template; every template adapts to light and dark themes.
