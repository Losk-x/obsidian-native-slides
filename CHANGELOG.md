...
- **WYSIWYG mode** (deck notes only): explicit immersive mode — command `Toggle WYSIWYG Mode` (default hotkey `Mod+Shift+E`), bottom-bar button, or settings toggle (default off). When on, the **tab bar and sidebars hide** in **Live Preview and reading view** (Source mode stays completely native), the bottom bar also shows in Live Preview and matches the tab bar's measured height (runtime CSS variable; no content-area height change when switching modes), and **in-note properties hide while editing** in Live Preview (same look as reading view). Toggling from reading view jumps into the WYSIWYG edit view.
- **WYSIWYG zero typography intervention**: Obsidian's native Live Preview and reading view already share the same typography, so WYSIWYG never overrides it — only **standalone image lines are centered** in both views. The **Debug: Dump Typography Styles** command prints the current view's key computed styles and CSS variables to the console for measurement (Live Preview renders only the visible area — scroll to the element first).
