...
- **Style templates**: six built-in looks for the Slides card **and** slides bar — _Dashed outline_ (default, the previous look), _Paper card_ (solid slide with soft shadow), _Minimal_ (no card boundary, flat canvas), _Accent edge_ (accent line on card top and bar), _Frosted glass_ (translucent, blurred surfaces over a tinted backdrop) and _Lecture (wiki)_ (a measure-for-measure take on the slideshow cards of jyywiki.cn lecture notes: 600px bordered card with soft shadow, a compact bold title over a thin bottom rule, tight headings, disc bullets and 8px-per-level lists). Pick one under Settings → Style template; every template adapts to light and dark themes. The example vault ships a small deck (`jyy-overview`) that reproduces two jyywiki slides for side-by-side comparison.
