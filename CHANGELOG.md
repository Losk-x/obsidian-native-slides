...
- Slides mode now **scales proportionally with the window** (CSS `zoom`) to keep the card within one screen — no scrolling. The scale is anchored to the viewport at entry, so resizing / Obsidian zoom scales it bidirectionally; content past one screen shrinks to fit down to a 60% floor, then clips.
