...
- **WYSIWYG minimal intervention**: under WYSIWYG mode, default typography is deliberately left untouched (Obsidian's edit and reading views already look nearly identical; other themes/plugins may restyle) — only **standalone image lines are centered** in both views. The **Debug: Dump Typography Styles** command prints the current view's key computed styles and CSS variables to the console for measurement (edit view renders only the visible area — scroll to the element first).
