...
- Slides mode now **auto-shrinks to fit one screen** (CSS `zoom`): content that fits stays at its natural size — adding text no longer re-scales it — and overflowing content shrinks down to a 60% floor then clips. Obsidian's own zoom (Cmd/Ctrl+=/-) scales the card natively and is not intercepted.
