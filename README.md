...
- **WYSIWYG mode** (_deck notes only_): an explicit immersive mode (command **Toggle WYSIWYG Mode**, default hotkey `Mod+Shift+E`, a bottom-bar button, or the settings toggle, default off). When on: the **tab bar and sidebars hide** in **Live Preview and reading view** (Source mode stays completely native), the bottom bar also shows in Live Preview and matches the tab bar's measured height (no content-area height change when switching modes), the **in-note properties panel hides while editing**, and **standalone image lines are centered** in both views. **Zero typography intervention**: Obsidian's native Live Preview and reading view already share the same typography, so the plugin never overrides it (themes/plugins stay compatible).
