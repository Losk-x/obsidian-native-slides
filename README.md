...
- **Slides mode** (_deck notes only_): an immersive, editable card view — "one screen = one card". Enter with the **Toggle Slides Mode** command (default hotkey `Mod+Shift+E`); the ribbon, sidebars and tab bar hide, the editor **auto-shrinks to fit one screen** (content that fits stays at its natural size; overflowing content shrinks down to 60% then clips; Obsidian's own zoom scales the card natively), and a bottom bar shows the note's properties, ◀ ▶ navigation and the auto-computed page number. Exiting restores the view you came from (Source / Live Preview / Reading).
