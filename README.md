...
- **WYSIWYG mode** (_deck notes only_): an explicit immersive mode (command **Toggle WYSIWYG Mode**, default hotkey `Mod+Shift+E`, a bottom-bar button, or the settings toggle, default off). When on: the **tab bar and sidebars hide** in both edit and reading views, the bottom bar also shows in edit view and matches the tab bar's measured height (no content-area height change when switching modes), the **in-note properties panel hides while editing**, and **standalone image lines are centered** in both views. Default typography is deliberately left untouched — Obsidian's edit and reading views already look nearly identical, and other themes/plugins may restyle text, formulas and code blocks (minimal-intervention principle). Outside the mode, edit view behaves natively (properties show normally).
- **Commands**: _Toggle Properties Bar_ and _Pause/Resume Auto Fullscreen_ (both persist), plus _Previous Page / Next Page_ for deck navigation, _Create Next Slide_ for deck authoring, and _Toggle WYSIWYG Mode_ — all rebindable under _Settings → Hotkeys_. _Debug: Dump Typography Styles_ prints the current view's key typography metrics to the console (run it in edit and reading view and compare — used to tune the WYSIWYG typography alignment).
