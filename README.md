...
- **Slides mode** (_deck notes only_): an immersive, editable card view — "one screen = one card". Enter with the **Toggle Slides Mode** command (default hotkey `Mod+Shift+E`); the ribbon, sidebars and tab bar hide, it auto-enters **OS fullscreen** (toggleable in settings), the editor **clips to a single screen** (no scrolling — content beyond the fold is clipped), and a bottom bar shows the note's properties, ◀ ▶ navigation and the auto-computed page number. Exiting restores the view you came from (Source / Live Preview / Reading).
