...
- **WYSIWYG 一步**：对 deck（卡片）笔记，在实时预览中**折叠**的属性面板会被彻底隐藏——与阅读模式观感一致（用命令面板 _Toggle fold properties in current file_ 或源码模式展开）。可在设置中开关。
...
演示笔记位于 [`example-vault/`](example-vault/)，这就是要打开的 Obsidian 示例库。它包含 `overview.md`、`welcome.md`、`slide-2.md`、`slide-3.md`、`broken-link-demo.md`（断链警告演示）、`folded-properties-demo.md`（WYSIWYG 折叠属性演示）、一份最小化的 `.obsidian/` 配置，以及一个插件目录 `example-vault/.obsidian/plugins/native-slides/`，其中的文件（`manifest.json`、`main.js`、`styles.css`）都是**指向仓库根目录的符号链接**——示例库始终运行当前构建。
