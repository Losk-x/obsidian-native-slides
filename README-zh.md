...
- **WYSIWYG 模式**（仅 deck 笔记）：显式模式——命令 **Toggle WYSIWYG Mode**（默认快捷键 `Mod+Shift+E`）、底栏按钮或设置开关进入，统一编辑与阅读的排版观感。开启后**编辑视图也显示底栏**；在阅读视图切换时自动跳入 WYSIWYG 编辑视图。**阅读视图**中顶部 tab bar 隐藏，底栏高度自动对齐 tab bar 实测高度——切换模式时内容区高度不变（排版对齐仍在后续阶段）。
