...
- **演示时没有闪烁光标**：点一下 slides 栏即可让编辑器失焦——讲解时不再有闪烁的输入光标；点回任意幻灯片内容即可继续编辑。
...
- **命令**：_Toggle Slides Mode_（`Mod+Shift+E`）、_Previous Page / Next Page_、_Create Next Slide_、_Toggle Slides Bar_——都可在 _设置 → 快捷键_ 重新绑定。
